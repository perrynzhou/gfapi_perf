 ...
 *
 * to compile: gcc -pthread -g -O0  -Wall --pedantic -o gfapi_perf_tool -I /usr/include/glusterfs/api gfapi_perf.c  -lgfapi -lrt
 *
...
void * gfapi_thread_run( void * void_result_p )
{
  ...
  FOREACH(k, prm.filecount) {
   ...
   FOREACH( io_count, prm.io_requests ) {
    if (workload == WL_SEQWR) {
      ...
      bytes_xferred = prm.use_fuse ?
        ...
        glfs_pwrite(glfs_fd_p, buf, prm.bytes_to_xfer, offset, 0,NULL,NULL);
      if (bytes_xferred < prm.bytes_to_xfer) 
      ...
      bytes_xferred = prm.use_fuse ? 
        ...
        glfs_pread(glfs_fd_p, buf, prm.bytes_to_xfer, offset, 0,NULL);
      if (bytes_xferred < prm.bytes_to_xfer) 
                scallerr(prm.use_fuse?"pwrite":"glfs_pwrite");
    }
   ...
   if ((workload == WL_SEQWR || workload == WL_RNDWR) && prm.fsync_at_close) {
     rc = prm.use_fuse ? fsync(fd) : glfs_fsync(glfs_fd_p,NULL,NULL);
